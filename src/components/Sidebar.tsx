	...
	const settingsNavItems = React.useMemo(
		() => [
			...
			},
			{
				...
				to: "/settings/aria2-options",
			},
			{
				key: "aria2-general",
				label: "Aria2 General",
				icon: <IconGear className="w-4 h-4" />,
				to: "/settings/aria2-general",
			},
			{
				key: "aria2-network",
				label: "Network",
				icon: <IconNodesDown className="w-4 h-4" />,
				to: "/settings/aria2-network",
			},
			{
				key: "advanced",
				label: "Advanced",
		...
		<div className="flex flex-col h-full w-full">
			...
			<ScrollShadow className="flex-1 px-3 mt-4 overflow-y-auto">
				...
				>
					{mainNavItems.map((item) => (
						<ListBox.Item
							...
							onPress={onClose}
							className={cn(
								"px-4 py-3 rounded-2xl data-[hover=true]:bg-default-100/50 transition-colors",
				...
				<Accordion
					...
					className="px-0"
				>
					<Accordion.Item key="settings" aria-label="Settings" className="px-0">
						<Accordion.Heading>
							<Accordion.Trigger className="px-4 py-3 rounded-2xl data-[hover=true]:bg-default-100/50">
								<div className="flex items-center gap-3">
									<IconGear className="w-5 h-5 text-default-500" />
									<span className="text-sm font-bold tracking-tight">
										Settings
									</span>
								</div>
							...
							>
								{settingsNavItems.map((item) => (
									<ListBox.Item
										...
										onPress={onClose}
										className={cn(
											"px-4 py-2 rounded-xl data-[hover=true]:bg-default-100/50 transition-colors",
											selectedKey === item.key &&
												"bg-default-100 text-foreground font-bold",
										)}
									>
										<div className="flex items-center gap-3">
...
export const Sidebar: React.FC = () => {
	return (
		<aside className="w-64 border-r border-default-100 bg-default-50/30 hidden md:flex flex-col h-full shrink-0">
			<SidebarContent />
		</aside>
	);
};
